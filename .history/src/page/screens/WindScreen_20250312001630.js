...
const WindScreen = () => {
  ...
  const currentWindSpeedMs = 20; // m/s
  const currentWindSpeedKmh = currentWindSpeedMs * 3.6; // Chuyển sang km/h
  const [windLevel, setWindLevel] = useState("Light Breeze");
  // Thêm state cho hiệu ứng trượt (slide)
  const [warningSlideX] = useState(new Animated.Value(200));
  ...
  const [iconName, setIconName] = useState("weather-windy");
  ...
  useEffect(() => {
    if (currentWindSpeedKmh > 60) {
      ...
      Animated.parallel([
        ...
        Animated.timing(warningSlideX, {
          toValue: 0, // trượt vào vị trí 0
          duration: 500,
          useNativeDriver: true,
        }),
      ]).start();
      ...
      setIconName("weather-windy-variant");
    ...
    >
      <View style={styles.overlay}>
        ...
        <Animated.View style={[styles.iconContainer, shakeStyle]}>
          ...
          /> */}
          {/* <Ionicons name={iconName} size={120} color={"#FF4444"} /> */}
          <MaterialCommunityIcons name={iconName} size={120} color="#4F8EF7" />
        </Animated.View>
